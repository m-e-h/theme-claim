...
let getConf = require("./configPath");
let themeRoot = getConf.themeRoot;

const doReplacePhp = async config => {
	let conf = await fs.readJson(config);
...
const doReplaceCss = async config => {
	let conf = await fs.readJson(config);
...
const doReplaceJs = async config => {
	let conf = await fs.readJson(config);
...
const doReplaceJson = async config => {
	let conf = await fs.readJson(config);
